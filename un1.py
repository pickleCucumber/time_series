...
df = pd.read_csv('ICAAP_daily BG2_irrbb_fact.csv', sep=';',encoding='PT154')
dc=df.melt(id_vars=['date'])
dc['date']=pd.to_datetime(dc['variable'], format='%d.%m.%Y')
del dc['variable']
...
ax = plt.subplot(211)
sm.graphics.tsa.plot_acf(avg.value_box_diff2[13:].values.squeeze(), lags=14, ax=ax)
plt.show()
ax = plt.subplot(212)
sm.graphics.tsa.plot_pacf(avg.value_box_diff2[13:].values.squeeze(), lags=14, ax=ax)
plt.show()
...
avg['model'] = invboxcox(best_model.fittedvalues, lmbda)
...
df2['forecast'] = invboxcox(best_model.predict(start=30, end=50), lmbda)
